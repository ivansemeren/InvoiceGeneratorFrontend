...
export class InvoiceFormComponent implements OnInit {
  ...
  private setInvoiceToForm() {
    this.route.params
      .subscribe(params => {
        ...
        this.title = 'Edit Invoice';     // way without resolve
        // this.invoiceService.getInvoice(id).subscribe(invoice => {
        //   this.invoice = invoice;
        //   this.invoiceForm.patchValue(this.invoice);
        // }, err => this.errorHandler(err, 'faoled to get invoice'));
  ...
  onSubmit() {
    if (this.invoice) {
      ...
      }, err => this.errorHandler(err, 'Falied to Update'));
    } else {
      this.invoiceService.createInvoice(this.invoiceForm.value)
      .subscribe(data => {
        this.snackBar.open('Successfuly created', 'Success', {
          duration: 2000,
        });
        this.router.navigate(['dashboard', 'invoices']);
